...
namespace ABI.CCK.Scripts.Editor
{
    ...
    public class CCK_CVRAvatarEditor : UnityEditor.Editor
    {
        ...
        private void InitializeList()
        {
            ...
            reorderableList = new ReorderableList(_avatar.avatarSettings.settings, typeof(CVRAdvancedSettingsEntry), true, true, true, true);
        ...
        private float OnHeightElement(int index)
        {
            ...
            switch (entity.type)
            {
                case CVRAdvancedSettingsEntry.SettingsType.GameObjectToggle:
                    ...
                    if (gameObjectToggle == null || gameObjectToggle.gameObjectTargets == null) return EditorGUIUtility.singleLineHeight * 8f;
                    return EditorGUIUtility.singleLineHeight * (5f + 3f + (3.75f * Mathf.Max(gameObjectToggle.gameObjectTargets.Count, 1f)));
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.GameObjectDropdown:
                    var gameObjectDropdown = (CVRAdvancesAvatarSettingGameObjectDropdown) entity.setting;
                    if (gameObjectDropdown == null || gameObjectDropdown.options == null) return EditorGUIUtility.singleLineHeight * 8f;
                    var height = 5f + 3.75f;
                    foreach (var option in gameObjectDropdown.options)
                    {
                        height += 5f;
                        height += Mathf.Max(option.gameObjectTargets.Count, 1f) * 2.5f;
                    }
                    return EditorGUIUtility.singleLineHeight * height;
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.MaterialColor:
                    var materialColor = (CVRAdvancedAvatarSettingMaterialColor) entity.setting;
                    if (materialColor == null || materialColor.materialColorTargets == null) return EditorGUIUtility.singleLineHeight * 8f;
                    return EditorGUIUtility.singleLineHeight * (5f + 3f + (3.75f * Mathf.Max(materialColor.materialColorTargets.Count, 1f)));
                    break;
                case CVRAdvancedSettingsEntry.SettingsType.Slider:
                    var slider = (CVRAdvancesAvatarSettingSlider) entity.setting;
                    if (slider == null || slider.materialPropertyTargets == null) return EditorGUIUtility.singleLineHeight * 8f;
                    return EditorGUIUtility.singleLineHeight * (5f + 3f + 4f + (6.25f * Mathf.Max(slider.materialPropertyTargets.Count, 1f)));
                    break;
            }
        ...
        private void OnDrawElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            switch (entity.type)
            {
                ...
                case CVRAdvancedSettingsEntry.SettingsType.Slider:
                    ...
                    rect.y += Math.Max(materialPropertyList.count, 1) * EditorGUIUtility.singleLineHeight * 6.25f + EditorGUIUtility.singleLineHeight * 3f;
